...
# title:           4 exploratory analyses.R
#
# description:
# ...
# ---->>>>---->>>>---->>>>---->>>>---->>>>---->>>>---->>>>---->>>>---->>>>
...
out_music <- map_df(list.files(path = "mplus/music/", 
                                  pattern = "model_[123].out", 
                                  full.names = T), 
                       ~tidy_mplus(.)) %>% mutate(analysis = "music")
out_music_24h <- map_df(list.files(path = "mplus/music_24h/", 
                                  pattern = "model_[123].out", 
                                  full.names = T), 
                       ~tidy_mplus(.)) %>% mutate(analysis = "music_24h")
out_music_99th <- map_df(list.files(path = "mplus/music_99th/", 
                                  pattern = "model_[123].out", 
                                  full.names = T), 
                       ~tidy_mplus(.)) %>% mutate(analysis = "music_99th")
...
combined <- bind_rows(out_game, out_game_24h, out_game_99th) %>% 
  bind_rows(out_music, out_music_24h, out_music_99th) %>% 
  bind_rows(out_social, out_social_24h, out_social_99th) %>% 
  bind_rows(out_video, out_video_24h, out_video_99th) %>%  
  filter(str_detect(effect, "[Vv]ariances|[Pp]hi|effe", negate = T)) %>% 
  arrange(desc(BW), analysis) %>% 
  relocate(effect, analysis, random_fixed, BW) %>% 
  mutate_all(as.character) %>% 
  replace(is.na(.), "") %>% 
  arrange(random_fixed, desc(BW), effect %>% 
            str_remove("Sleep on |Sleep with ") %>% 
            factor(levels = c("Daytime", "Prebed", "Postbed"))) %>% 
  mutate(effect = str_replace(effect, "on", "\u2192"),
         effect = str_replace(effect, "with", "\u2194"))

combined <- combined %>% 
  # filter(str_detect(analysis,"game"))
  # filter(str_detect(analysis,"social"))
  # filter(str_detect(analysis,"video"))
  filter(str_detect(analysis,"music"))
