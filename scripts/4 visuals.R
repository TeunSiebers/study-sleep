...
# creat plot
prep_figure_1 <- timeframe_per_cat %>% 
  mutate(cat = case_when(cat == "phone" ~ "Smartphone",
                         cat == "social" ~ "Social media app",
                         cat == "game" ~ "Game app",
  ...
  mutate(timeframe = case_when(timeframe == "daily_use" ~ "Daily use",
                               timeframe == "daytime_use" ~ "Daytime use",
                               timeframe == "prebed_use" ~ "Pre-bedtime use",
                               timeframe == "postbed_use" ~ "Post-bedtime use"))

# create ggrides plot
figure_1 <- prep_figure_1 %>%
  ggplot() +
  geom_density_ridges(aes(x = duration, y = cat, fill = cat, color = cat), 
                      alpha = 0.9, scale = 0.9) +
  scale_fill_manual(breaks = c("Smartphone", "Social media app", "Game app", "Video player app"),
                    values = c("Social media app" = "#FC4E07",
                               "Game app" = "#E7B800",
                               "Video player app" = "#00AFBB",
                               "Smartphone" = "grey39")) +
  scale_color_manual(breaks = c("Smartphone", "Social media app", "Game app", "Video player app"),
                     values = c("Social media app" = "#FC4E07",
                                "Game app" = "#E7B800",
                                "Video player app" = "#00AFBB",
                                "Smartphone" = "grey39")) +
  scale_x_continuous(name = "Average time spent (in hours)", 
                     breaks = pretty_breaks(3), limits = c(0, NA)) + 
  scale_y_discrete(name = NULL, limits=rev,expand = c(0, 0)) +
  labs(y = "Density", fill = "App category:", color = "App category:") + 
  theme_minimal() +
  theme(panel.spacing = unit(1.3,'lines'),
        legend.position = "none", legend.justification = "right",
        panel.grid.minor.y = element_blank())

design <- matrix(c(1, 1, 2, 4, 3), 1, 5)
layout(design)
par(mar = c(2,2,1,1))

figure_1 <- figure_1 + 
  facet_manual(vars(factor(timeframe)), design = design, scales = "free_x") +
  theme(plot.margin = grid::unit(c(1,1,1,1), "mm"))

figure_1

# save plot
if(F) ggsave(plot = figure_1, "data/output/figures/Figure 1 - density.svg", width = 22, height = 8, units = "cm")
...
# strip whitespace
figure_2 <- figure_2 + theme(plot.margin = grid::unit(c(0,6,1,1), "mm"))
figure_2

# save plot
if(F) ggsave(plot = figure_2, "data/output/figures/Figure 2 - app use.svg", width = 22, height = 11, units = "cm")
